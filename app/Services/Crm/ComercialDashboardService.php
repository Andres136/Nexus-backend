...
class ComercialDashboardService
{
    public function getMesAMes(?int $userId, string $inicio): array
    {
        ...
        foreach ($resultado as $r) {
            ...
            $agrupadoTrimestre[$key]['cotizaciones'] = ($agrupadoTrimestre[$key]['cotizaciones'] ?? 0) + $r['cotizaciones'];
            $agrupadoTrimestre[$key]['ordenes']      = ($agrupadoTrimestre[$key]['ordenes']      ?? 0) + $r['ordenes'];
        }

        foreach ($resultado as &$r) {
            $key = $r['user_id'] . '_' . $r['trimestre'];
            $agg = $agrupadoTrimestre[$key];
            $r['conversion_trimestral_pct'] = $agg['cotizaciones'] > 0
                ? round(($agg['ordenes'] / $agg['cotizaciones']) * 100, 2)
                : 0;
        }
        unset($r);
